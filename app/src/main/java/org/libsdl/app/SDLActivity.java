...
public class SDLActivity extends Activity {
    ...
    protected void onResume() {
        ...
        if (requestAudioFocus()) {
            ...
            Log.w(TAG, "Audio focus not granted on resume; keeping audio paused");
            SDLActivity.handlePause();
        }
    }
    ...
    private static void applyAudioDuck(boolean duck) {
        if (mAudioTrack == null) {
            return;
        }

        float volume = duck ? 0.2f : 1.0f;
    ...
    public static native void onNativeMouseRelative(float dx, float dy);
...
class SDLSurface extends SurfaceView implements SurfaceHolder.Callback,
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        ...
        if (SDLActivity.mExitCalledFromJava && SDLActivity.mSDLThread == null) {
...
class SDLInputConnection extends BaseInputConnection {
    ...
    public boolean commitText(CharSequence text, int newCursorPosition) {
        ...
        // Also synthesize key events for each character so apps like DOSBox 
        // that rely on SDL_KEYDOWN/UP events will receive them
        for (int i = 0; i < text.length(); i++) {
            char c = text.charAt(i);
            sendCharacterAsKeyEvent(c);
        }

        return super.commitText(text, newCursorPosition);
    }

    private void sendCharacterAsKeyEvent(char c) {
        int keyCode = getKeyCodeForChar(c);
        boolean needsShift = needsShiftForChar(c);
    ...
    private boolean needsShiftForChar(char c) {
    ...
    private int getKeyCodeForChar(char c) {
        // Map characters to key codes (unshifted versions)
