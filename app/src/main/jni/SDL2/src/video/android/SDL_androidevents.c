...
static int AndroidLifecyclePaused = 0;
#if SDL_ANDROID_BLOCK_ON_PAUSE
static int AndroidLifecyclePausing = 0;
#endif

void
Android_ResetLifecycleState(void)
{
    AndroidLifecyclePaused = 0;
#if SDL_ANDROID_BLOCK_ON_PAUSE
    AndroidLifecyclePausing = 0;
#endif
}
...
#if SDL_ANDROID_BLOCK_ON_PAUSE
    if (AndroidLifecyclePaused && !AndroidLifecyclePausing) {
        /* Make sure this is the last thing we do before pausing */
        android_egl_context_backup();
        ANDROIDAUDIO_PauseDevices();
        if(SDL_SemWait(Android_ResumeSem) == 0) {
#else
    if (AndroidLifecyclePaused) {
...
#if SDL_ANDROID_BLOCK_ON_PAUSE
        if( AndroidLifecyclePausing || SDL_SemTryWait(Android_PauseSem) == 0 ) {
            ...
            if (SDL_HasEvent(SDL_WINDOWEVENT) || SDL_HasEvent(SDL_APP_WILLENTERBACKGROUND) || SDL_HasEvent(SDL_APP_DIDENTERBACKGROUND) ) {
                AndroidLifecyclePausing = 1;
            }
            else {
                AndroidLifecyclePausing = 0;
                AndroidLifecyclePaused = 1;
            }
        }
#else
