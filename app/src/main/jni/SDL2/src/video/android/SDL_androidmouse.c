...
/* New function for proper relative mouse motion (for trackpad/mouse emulation) */
void Android_OnMouseRelative(float dx, float dy) {
    if (!Android_Window) {
        __android_log_print(ANDROID_LOG_WARN, "SDL", "Android_OnMouseRelative: No window! dx=%.1f, dy=%.1f", dx, dy);
        return;
    }
    
    __android_log_print(ANDROID_LOG_VERBOSE, "SDL", "Android_OnMouseRelative: dx=%.1f, dy=%.1f -> sending (%d, %d)", dx, dy, (int)dx, (int)dy);
    
    /* Send relative motion - the third parameter (1) indicates relative mode */
    /* Cast floats to ints as SDL_SendMouseMotion expects int parameters */
    SDL_SendMouseMotion(Android_Window, 0, 1, (int)dx, (int)dy);
}
