...
GLES2_CreateRenderer(SDL_Window *window, Uint32 flags)
{
    ...
    SDL_Log("SDL:GLES2_CreateRenderer start window=%p flags=0x%x windowFlags=0x%x", (void*)window, (unsigned int)flags, (unsigned int)SDL_GetWindowFlags(window));
    ...
    if (!(window_flags & SDL_WINDOW_OPENGL) ||
        ...
        SDL_Log("SDL:GLES2_CreateRenderer recreating window windowFlags=0x%x profile=%d major=%d minor=%d", (unsigned int)window_flags, profile_mask, major, minor);
        SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_ES);
        SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, RENDERER_CONTEXT_MAJOR);
        SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, RENDERER_CONTEXT_MINOR);

        if (SDL_RecreateWindow(window, window_flags | SDL_WINDOW_OPENGL) < 0) {
            SDL_Log("SDL:GLES2_CreateRenderer SDL_RecreateWindow failed: %s", SDL_GetError());
            goto error;
        }
    }
    ...
    SDL_Log("SDL:GLES2_CreateRenderer creating GL context");
    data->context = SDL_GL_CreateContext(window);
    if (!data->context) {
        SDL_Log("SDL:GLES2_CreateRenderer SDL_GL_CreateContext failed: %s", SDL_GetError());
        GLES2_DestroyRenderer(renderer);
        goto error;
    }
    SDL_Log("SDL:GLES2_CreateRenderer created GL context=%p", data->context);
    if (SDL_GL_MakeCurrent(window, data->context) < 0) {
        SDL_Log("SDL:GLES2_CreateRenderer SDL_GL_MakeCurrent failed: %s", SDL_GetError());
        GLES2_DestroyRenderer(renderer);
        goto error;
    }
    SDL_Log("SDL:GLES2_CreateRenderer made context current");

    if (GLES2_LoadFunctions(data) < 0) {
        SDL_Log("SDL:GLES2_CreateRenderer GLES2_LoadFunctions failed: %s", SDL_GetError());
        GLES2_DestroyRenderer(renderer);
        goto error;
    }
    SDL_Log("SDL:GLES2_CreateRenderer loaded functions");
    ...
    SDL_Log("SDL:GLES2_CreateRenderer success context=%p", data->context);

    return renderer;

error:
    SDL_Log("SDL:GLES2_CreateRenderer error changedWindow=%d err=%s", changed_window ? 1 : 0, SDL_GetError());
    if (changed_window) {
