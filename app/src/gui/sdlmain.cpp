...
extern std::atomic<bool> g_embedded_session_mode;

static SCREEN_TYPES g_last_window_screen_type = SCREEN_SURFACE;

void DOSBox_ResetSDLWindowModeState() {
	g_last_window_screen_type = SCREEN_SURFACE;
}

void DOSBox_DestroySDLVideoObjects() {
	/* Embedded AA relaunches can arrive on the Java/UI thread after SDL has already
	 * started tearing down its GL objects. Clearing stale pointers here is safer than
	 * calling SDL_Destroy* from this reset hook, which can crash in SDL_GL_MakeCurrent.
	 */
	sdl.renderer = 0;
...
static SDL_Window * GFX_SetSDLWindowMode(Bit16u width, Bit16u height, bool fullscreen, SCREEN_TYPES screenType) {
	bool use_texture_gl_window = false;
	#ifdef __ANDROID__
	use_texture_gl_window = g_embedded_session_mode.load() && (screenType == SCREEN_TEXTURE);
	#endif
	LOG_MSG("SDL:GFX_SetSDLWindowMode width=%u height=%u fullscreen=%d screenType=%d lastType=%d hasWindow=%d updateWindow=%d", (unsigned int)width, (unsigned int)height, fullscreen ? 1 : 0, (int)screenType, (int)g_last_window_screen_type, sdl.window ? 1 : 0, sdl.update_window ? 1 : 0);
	if (sdl.renderer) {
	 ...
	 */
	if (!sdl.window
	    || (g_last_window_screen_type != screenType)
...
//	    || (fullscreen && ((width != currWidth) || (height != currHeight)))
		...
		g_last_window_screen_type = screenType;
		if (sdl.window) {
			LOG_MSG("SDL:Destroying existing SDL window before recreation");
		...
		sdl.window = SDL_CreateWindow("",
		                 ...
		                 | (((screenType == SCREEN_OPENGL) || use_texture_gl_window) ? SDL_WINDOW_OPENGL : 0) | SDL_WINDOW_SHOWN);
		LOG_MSG("SDL:SDL_CreateWindow returned %p flags=0x%x error=%s", (void*)sdl.window, (unsigned int)SDL_GetWindowFlags(sdl.window), sdl.window ? "" : SDL_GetError());
		if (sdl.window)
...
dosurface:
	...
	case SCREEN_TEXTURE:
	{
		...
		LOG_MSG("SDL:About to create renderer driver=%s window=%p flags=0x%x", sdl.rendererDriver, (void*)sdl.window, (unsigned int)SDL_GetWindowFlags(sdl.window));
		...
		for (Bit32u ct = 0,startticks = GetTicks();ct < max_splash_loop;ct = GetTicks()-startticks) {
			...
			if (ct<1) {
				...
				if (use_fadeout) {
					...
					SDL_BlitScaled(splash_surf, NULL, sdl.surface, NULL);
#else
...
//					}
		...
		case SDL_MOUSEMOTION:
			// WiFi controller trackpad sends relative mouse motion
			Mouse_CursorMoved((float)event.motion.xrel*sdl.mouse.sensitivity/100.0f,
			                  (float)event.motion.yrel*sdl.mouse.sensitivity/100.0f,
			                  0, 0, true);
			break;
		case SDL_MOUSEBUTTONDOWN:
		case SDL_MOUSEBUTTONUP:
			// WiFi controller mouse buttons
