...
static Uint32 wifi_joystick_last_update_ms = 0;

void MAPPER_SetVirtualJoystickAxis(float x, float y) {
	if (x > 1.0f) x = 1.0f;
	if (x < -1.0f) x = -1.0f;
	if (y > 1.0f) y = 1.0f;
	if (y < -1.0f) y = -1.0f;

	const Bit16s sx = (Bit16s)(x * 32767.0f);
	const Bit16s sy = (Bit16s)(y * 32767.0f);

	/* 2-axis groups read [0]/[1] from virtual_joysticks[emustick] */
	virtual_joysticks[0].axis_pos[0] = sx;
	virtual_joysticks[0].axis_pos[1] = sy;
	virtual_joysticks[1].axis_pos[0] = sx;
	virtual_joysticks[1].axis_pos[1] = sy;

	/* 4-axis style groups read [0..3] from virtual_joysticks[0] */
	virtual_joysticks[0].axis_pos[2] = sx;
	virtual_joysticks[0].axis_pos[3] = sy;

	wifi_joystick_last_update_ms = SDL_GetTicks();
}
...
void MAPPER_UpdateJoysticks(void) {
	/*
	 * WiFi joystick injects axis values directly from Android networking code.
	 * Mapper polling can overwrite those values every frame, so briefly skip
	 * polling while WiFi updates are actively arriving.
	 */
	if (wifi_joystick_last_update_ms != 0 &&
	    (SDL_GetTicks() - wifi_joystick_last_update_ms) < 250) {
		return;
	}
